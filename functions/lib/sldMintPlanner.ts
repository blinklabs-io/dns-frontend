...
import { assetId, createReferenceTokenTN, createUserTokenTN, prepareSldMintArtifacts, type PreparedArtifacts } from "./sldBuilder";
import { normalizeAssetId, splitAssetId, MIN_LOVELACE, isValidPolicyId } from "./cardano";
 ...
 */
async function toMeshUtxo(utxo: UTxO): Promise<MeshUtxo> {
  ...
  if (utxo.output.plutusData) {
    try {
      ...
      const safeTxHash = sanitizeForLog(utxo.input.txHash);
...
export async function checkSldAvailability({
  ...
  const tldRefUtxos = await Promise.all(rawTldRefUtxos.map(toMeshUtxo));
...
export async function buildSldMintPlan({
  ...
  if (!isValidPolicyId(csTld)) {
    throw new Error(`Invalid TLD policy ID: expected 56 hex characters, got "${sanitizeForLog(csTld)}"`);
  }
  if (!isValidPolicyId(csSld)) {
    throw new Error(`Invalid SLD policy ID: expected 56 hex characters, got "${sanitizeForLog(csSld)}"`);
  }
  ...
  const [userUtxos, ownerUtxos, tldRefUtxos] = await Promise.all([
    Promise.all(rawUserUtxos.map(toMeshUtxo)),
    Promise.all(rawOwnerUtxos.map(toMeshUtxo)),
    Promise.all(rawTldRefUtxos.map(toMeshUtxo)),
  ]);
  ...
  let effectiveSldHexList = (currentSldHexList ?? []).map((h) => h.toLowerCase());
  ...
  if (effectiveSldHexList.includes(sldHex)) {
    throw new Error(`SLD '${sanitizeForLog(sldName)}' already exists under this TLD`);
  }
