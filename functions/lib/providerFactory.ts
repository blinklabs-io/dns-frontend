      ...
      if (asset && result) {
        ...
        result = (result as UTxO[]).filter((utxo) =>
          utxo.output.amount.some(
            (a) => normalizeAssetId(a.unit) === normalizedAsset,
          ),
        );
      }
    } else {
...
// Module-level cache — Workers reuse isolates so this persists across requests
// within the same isolate lifetime. Keyed on env values so stale providers are
// not silently returned when env bindings change (e.g. across deployments).
let cachedProvider: { key: string; adapter: MeshProviderAdapter } | null = null;

function envCacheKey(env: Env): string {
  return `${env.MESH_PROVIDER ?? ""}|${env.BLOCKFROST_PROJECT_ID ?? ""}|${env.BLOCKFROST_URL ?? ""}|${env.BLOCKFROST_VERSION ?? ""}|${env.KUPO_URL ?? ""}|${env.OGMIOS_URL ?? ""}`;
}

/**
 * Creates or returns a cached provider adapter based on environment configuration.
 * Provider type is determined by MESH_PROVIDER env var.
 ...
 */
export function createProvider(env: Env): MeshProviderAdapter {
  const key = envCacheKey(env);
  if (cachedProvider?.key === key) return cachedProvider.adapter;
  const providerKind = (env.MESH_PROVIDER || "blockfrost")
    .toLowerCase()
    .trim();
  ...
  if (providerKind === "blockfrost") {
    adapter = new MeshProviderAdapter(async () => {
      ...
      const baseUrl = env.BLOCKFROST_URL;
      if (baseUrl) {
      ...
      if (!projectId)
        throw new Error(
          "BLOCKFROST_PROJECT_ID or BLOCKFROST_URL is required",
        );
      const rawVersion = env.BLOCKFROST_VERSION
        ? Number(env.BLOCKFROST_VERSION)
        : 0;
    ...
    adapter = new MeshProviderAdapter(async () => {
      ...
      if (!kupoUrl)
        throw new Error("KUPO_URL is required when MESH_PROVIDER=kupo");
    ...
    adapter = new MeshProviderAdapter(async () => {
      ...
      if (!ogmiosUrl)
        throw new Error("OGMIOS_URL is required when MESH_PROVIDER=ogmios");
    ...
    adapter = new MeshProviderAdapter(async () => {
      ...
      if (!kupoUrl)
        throw new Error(
          "KUPO_URL is required when MESH_PROVIDER=kupo+ogmios",
        );
      if (!ogmiosUrl)
        throw new Error(
          "OGMIOS_URL is required when MESH_PROVIDER=kupo+ogmios",
        );
      ...
      async function fetchAddressUTxOsImpl(
        address: unknown,
        asset?: string,
      ): Promise<unknown[]> {
        ...
        try {
          ...
          console.warn(
            "Kupo fetchAddressUTxOs failed, falling back to Ogmios:",
            sanitizeForLog(err),
          );
        }
        if (result === undefined) {
        ...
        if (result === undefined) {
          throw new Error(
            "Neither Kupo nor Ogmios supports fetchAddressUTxOs",
          );
        }
      ...
      return {
        ...
        async fetchUTxOs(hash: string, index?: number): Promise<unknown[]> {
          try {
            ...
            console.warn(
              "Kupo fetchUTxOs failed, falling back to Ogmios:",
              sanitizeForLog(err),
            );
          }
        ...
        async submitTx(tx: unknown): Promise<unknown> {
          ...
          try {
            ...
            ogmiosError = err;
            console.warn(
              "Ogmios submitTx failed, falling back to Kupo:",
              sanitizeForLog(err),
            );
          }
          if (typeof kupoProvider.submitTx === "function") {
            try {
              ...
              console.warn("Kupo submitTx also failed:", sanitizeForLog(kupoError));
        ...
        async fetchProtocolParameters(epoch?: number): Promise<unknown> {
          ...
          if (typeof ogmiosProv.fetchProtocolParameters === "function") {
            try {
              ...
              console.warn(
                "Ogmios fetchProtocolParameters failed, falling back to Kupo:",
                sanitizeForLog(err),
              );
            }
          }
          ...
          throw new Error(
            "Neither Ogmios nor Kupo supports fetchProtocolParameters",
          );
        },
        ...
        async fetchAssetAddresses(
          ...
          if (typeof kupoProv.fetchAssetAddresses === "function") {
            try {
              ...
              console.warn(
                "Kupo fetchAssetAddresses failed, falling back to Ogmios:",
                sanitizeForLog(err),
              );
            }
          }
          ...
          throw new Error(
            "Neither Kupo nor Ogmios supports fetchAssetAddresses",
          );
        },
        ...
        async fetchBlockInfo(hash: string): Promise<unknown> {
          ...
          if (typeof kupoProv.fetchBlockInfo === "function") {
            try {
              ...
              console.warn(
                "Kupo fetchBlockInfo failed, falling back to Ogmios:",
                sanitizeForLog(err),
              );
            }
          }
          ...
          throw new Error(
            "Neither Kupo nor Ogmios supports fetchBlockInfo",
          );
        },
        ...
        async fetchLatestBlock(): Promise<unknown> {
          ...
          if (typeof ogmiosProv.fetchLatestBlock === "function") {
            try {
              ...
              console.warn(
                "Ogmios fetchLatestBlock failed, falling back to Kupo:",
                sanitizeForLog(err),
              );
            }
          }
          ...
          throw new Error(
            "Neither Ogmios nor Kupo supports fetchLatestBlock",
          );
        },
      } as MeshProvider;
    });
